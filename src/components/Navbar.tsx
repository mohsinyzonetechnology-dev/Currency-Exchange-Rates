...
import { useNavigate, Link as RouterLink, Link, useLocation } from 'react-router-dom';
...
// ✅ Sidebar Row optimized (Memoized)
...
const drawerContent = (
    <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* ✅ Scroll Listener yahan lagaya hai */}
      <Box 
      ...
      >
        <List>
          ...
          {/* Loader indicator jab mazeed data ho */}
      ...
      <AppBar position="fixed" sx={{ zIndex: theme.zIndex.drawer + 1, bgcolor: '#1976d2' }}>
        <Toolbar sx={{ justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            {isMobile && <IconButton color="inherit" onClick={() => setMobileOpen(true)} sx={{ mr: 2 }}><MenuIcon /></IconButton>}
            <Typography variant="h6">💱 Exchange</Typography>
          </Box>
          <Box>
            <Button color="inherit" component={RouterLink} to="/dashbord" sx={{ mx: 1, textTransform: "none" }}>Dashboard</Button>
